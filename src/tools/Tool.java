...
public class Tool {
	...
	public static void getPageInformation(String tableName, HttpServletRequest request,
		...
		param = request.getParameter("newName");
		if (param == null || param.isEmpty())
			pageInformation.setNewName(null);
		else
			pageInformation.setNewName(param);

		param = request.getParameter("newTerm");
		if (param == null || param.isEmpty())
			pageInformation.setNewTerm(null);
		else
			pageInformation.setNewTerm(param);
	...
	// 生成表的查询语句
	public static String getSqlScore(PageInformation pageInformation, String type) {
		String sql = "";

		// 删除
		/*
		 * if (pageInformation.getIds() != null &&
		 * !pageInformation.getIds().isEmpty()) { sql += "delete * from" +
		 * pageInformation.getTableName().toLowerCase() + " where " +
		 * pageInformation.getTableName().toLowerCase() + "Id in (" + " " +
		 * pageInformation.getIds() + ") "; } else
	...
	public static void setPageInformation(Integer allRecordCount, PageInformation pageInformation) {
		...
		try {
			...
			totalPageCount = (int) Math.ceil(1.0 * allRecordCount / pageInformation.getPageSize());
