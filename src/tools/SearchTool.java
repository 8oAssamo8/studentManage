...
public class SearchTool {
	...
	// 自定义函数，学生表的查询条件
	public static String studentUser(HttpServletRequest request) {
		String searchSql = "";
		if ("byClassName".equals(request.getParameter("searchType"))) {
			searchSql = addStringAnd("classId", searchSql, request);
		} else if ("byStudentId".equals(request.getParameter("searchType"))) {
			searchSql = addStringFuzzyAnd("studentId", searchSql, request);
		}
		// searchSql = addStringAnd("type", searchSql, request);
		// searchSql = addStringFuzzyAnd("name", searchSql, request);
		// searchSql = addStringAnd("enable", searchSql, request);
		// searchSql = addDateAnd("registerDate", "lowDate", "upDate",
		// searchSql, request);
		// String a;
		return searchSql;
	}
