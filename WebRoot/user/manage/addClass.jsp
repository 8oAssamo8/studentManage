	...
	$(document).ready(function() { //资源加载后才执行的 代码，就放到这个函数中，jquery能保证网页所有资源（html代码，图片，js文件，css文件等）都加载后，才执行此函数
		$("#myform").submit(function() {
			$.ajax({
				type : "POST",
				dataType : "json",
				url : '/studentManage/servlet/ClassServlet?type1=addClass',
					...
					<div class="card-body">
						<form id="myform" method="post">
							<div class="form-group">
								<label class="col-form-label"><h5>班级编号</h5></label> <input
									class="form-control" id="classId" name="classId" type="text"
									placeholder="请输入班级编号" required="required" pattern="(\d){6}"
									title="班级编号只能由6位数字组成" maxlength=6>
							</div>
							<div class="form-group">
								<label class="col-form-label"><h5>班级名称</h5></label> <input
									class="form-control" id="className" name="className"
									type="text" placeholder="请输入班级名称" required="required">
							</div>
